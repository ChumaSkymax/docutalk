...
export function useVapi(book: IBook) {
  ...
  useEffect(() => {
    const handlers = {
      // Run this code (call-start: () => {}) when the voice call starts
      "call-start": () => {
        isStoppingRef.current = false; // We are NOT stopping anymore, we are just talking
        setStatus("starting"); // AI speaks first, wait for it or  Call has started, AI will speak first -so this update status using setStatus('starting')
        setCurrentMessage(""); // Clear the current or old AI message
        setCurrentUserMessage(""); // Clear the current or old user message

        // Start duration timer
        startTimeRef.current = Date.now(); //Remember the exact time call started eg.1700000000000 (timestamp)
        setDuration(0); //Reset duration to 0

        // Set up interval to update duration every second
        timerRef.current = setInterval(() => {
          //if (startTimeRef.current) this check is the start time exist
          if (startTimeRef.current) {
            const newDuration = Math.floor(
              (Date.now() - startTimeRef.current) / TIMER_INTERVAL_MS,
            ); // This calculate call duration, Current time - start time = how long call has been running
            setDuration(newDuration); // Update the duration state

            // Check if time limit is reached
            if (newDuration >= maxDurationRef.current) {
              getVapi().stop(); // Stop the call
              setLimitError(
